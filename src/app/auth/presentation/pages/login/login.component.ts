...
@Component({
  ...
  type="submit"
  [disabled]="cargando()"
  class="w-full flex items-center justify-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 focus:bg-indigo-800 text-white font-bold rounded-xl shadow-md transition-colors disabled:opacity-50"
>
  <span *ngIf="cargando()" class="animate-spin inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full mr-2"></span>
  <span>{{ cargando() ? 'Ingresando...' : 'Ingresar al Portal' }}</span>
</button>
            </form>
          </div>
        </div>
